...
sirprint::sirprint(const sirmanage& c, const sirmodel& _sir)
    : cfg(c.get_in_filepath())
    , sir(_sir)
{}
sirprint::sirprint(const sirmanage& c, const sirmodelextended& _sirs)
    : cfg(c.get_in_filepath())
    , sirs(_sirs)
{}
...
void sirprint::print_tostdout(const std::vector<sirdata>& results, int duration)
{
  ...
  for (int day = 0; day < duration; day++) {
    std::cout << "|" << std::setw(5) << day << "|" << std::setw(5)
              ...
              << results[day].get_inf() << "|" << std::setw(5)
              << results[day].get_rec();
