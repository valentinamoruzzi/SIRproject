...
namespace sirmodel {
    ...
    char sirmanage::get_output_type(return output_file)
    void sirmanage::set_output_type(char out_type){
        if (out_type != '\0') output_file = out_type;
        else throw "Error: file path is empty"
    }

    string sirmanage::read_row_fromfile(){
        string row;
        size_t pos = -1; // riguarda
        if(sr != NULL){
            getline(*sr,row);
            pos = row.find("=");
            if(pos != -1){
    ...
    void sirmanage::printdata(sirdata d){
        if (get_output_type() == 'S') cout << d.toString() << endl;
        else if (get_output_type() == 'F') *sw << d.toString() <<endl;
    }
