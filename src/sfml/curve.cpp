...
Curve::Curve()
    : color_(sf::Color::White), size_(sf::Vector2f(100, 100)), fill_(true),
      thickness_(3), limit_(100) {}

Curve::Curve(const Vector2f &size, const sf::Color &color)
    : color_(color), size_(size), fill_(true), thickness_(3), limit_(100) {
  setColor(color);
}
...
void Curve::prepare(sf::Vector2f &rangex, sf::Vector2f &rangey) {
  ...
  if (data_.size() > 1) {
    ...
    for (std::list<float>::iterator it = data_.begin(); it != data_.end();
         ++it) {
      // min
      if (*it < rangey.x) {
        rangey.x = *it;
    ...
    int i = 0;
    float x = 0;
    for (std::list<float>::reverse_iterator it = data_.rbegin();
         it != data_.rend(); ++it) {
      x = size_.x - (i++ * xoffset);
