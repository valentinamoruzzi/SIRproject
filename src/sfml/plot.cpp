...
void Plot::setSize(const sf::Vector2f &size) {
  ...
  graphSize_.x = size_.x - kBorderSize * 2 - size_.x * (kLeftSize / 100.0) -
                 size_.x * (kRightSize / 100.0);
  graphSize_.y = size_.y - kBorderSize * 2 - size_.y * (kTopSize / 100.0) -
  ...
  bg_.setSize(
      sf::Vector2f(size_.x - kBorderSize * 2, size_.y - kBorderSize * 2));
...
void Plot::draw(sf::RenderTarget &target, sf::RenderStates states) const {
  ...
  for (std::map<std::string, Curve>::const_iterator it = curves_.begin();
       it != curves_.end(); ++it)
...
void Plot::prepare() {
  ...
  for (std::map<std::string, Curve>::iterator it = curves_.begin();
       it != curves_.end(); ++it) {
...
void Plot::setFont(const std::string &filename) {
  ...
  title_.setPosition(
      sf::Vector2f(-title_.getLocalBounds().width / 2 + size_.x / 2, 0));
}
